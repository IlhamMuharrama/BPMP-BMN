...
const csvData = `id;kategoriId;kategori;nama
...
async function main() {
  ...
  const newItems = lines.map(line => {
    const [id, kategoriId, kategori, nama] = line.split(';');
    return {
      id: `${kategoriId}-${id}`,
      kategoriId,
      kategori,
      nama,
      supplier: 'PT Internal',
      satuan: 'Buah',
      lokasiRak: 'Rak A',
      stokSekarang: 0,
      stokMin: 5,
  ...
  newItems.forEach(item => {
    if (!categoriesMap.has(item.kategoriId)) {
      categoriesMap.set(item.kategoriId, {
        id: item.kategoriId,
        nama: item.kategori,
        deskripsi: `Kategori ${item.kategori}`
      });
    }
  });
  
  try {
    const syncRes = await fetch('http://localhost:3000/api/sync?force=1');
    let db = await syncRes.json();
    
    // REPLACE entirely to ensure exactly 241 items and 32 categories
    db.Barang = newItems;
    db.Kategori = Array.from(categoriesMap.values());
    
    const postRes = await fetch('http://localhost:3000/api/sync', {
        method: 'POST',
    ...
    if (postRes.ok) {
       console.log(`Successfully OVERWRITTEN database with exactly ${db.Barang.length} items and ${db.Kategori.length} categories!`);
    } else {
       console.error("Failed to post data:", await postRes.text());
    }
  } catch (err) {
